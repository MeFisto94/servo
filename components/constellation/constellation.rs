...
pub struct Constellation<Message, LTF, STF> {
    ...
    canvas_chan: (Sender<ConstellationCanvasMsg>, IpcSender<CanvasMsg>),
    ...
    STF: ScriptThreadFactory<Message = Message>,
{
    ...
    fn handle_shutdown(&mut self) {
        ...
        if let Err(e) = self.canvas_chan.0.send(ConstellationCanvasMsg::Exit) {
    ...
    fn handle_create_canvas_paint_thread_msg(
        ...
        if let Err(e) = self.canvas_chan.0.send(ConstellationCanvasMsg::Create {
        ...
        if let Err(e) = response_sender.send((self.canvas_chan.1.clone(), canvas_id)) {
